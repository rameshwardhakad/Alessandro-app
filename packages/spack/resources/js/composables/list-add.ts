...
export const useListAdd = () => {
  ...
  function addList() {
    ...
    // postQueue('/project-lists', list, function () {
    //   console.log('hit postQueue callback')
    // })

    axios
      .post(`/project-lists`, list)
      .then(({ data }) => {
        const listIndex = projectDetailStore.project.lists.findIndex((item) => item.temp_id === list.temp_id)
