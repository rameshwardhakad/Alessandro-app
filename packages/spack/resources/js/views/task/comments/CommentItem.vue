<template>
  <div class="group relative flex py-1">
    ...
    <div
      ...
      class="absolute flex items-center bg-white ltr:right-0 ltr:pl-4 rtl:left-0 rtl:pr-4"
      :class="{ 'opacity-50': processing }"
    >
      <span class="text-sm text-gray-600 ltr:mr-2 rtl:ml-2">{{
    ...
    <div
      v-else
      class="absolute hidden bg-white group-hover:flex ltr:right-0 ltr:pl-4 rtl:left-0 rtl:pr-4"
    >
...
<script setup lang="ts">
  ...
  function deleteComment() {
    ...
    axios.delete(`comments/${props.id}`).then(({ data }) => {
      ...
      comments.value.splice(props.index, 1)
    })
  }
</script>
