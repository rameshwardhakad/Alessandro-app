<template>
  <div class="space-y-1 px-2 py-4">
    <template v-for="item in useSidebarNav" :key="item.label">
      <RouterLink v-if="!item.permission || can(item.permission)" v-slot="{ href, navigate }" custom :to="item.uri">
        <a
          :href="href"
          :class="[
            isActive(item.uri) ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white',
            'group flex items-center rounded-md py-2 ltr:pl-3 rtl:pr-3 text-sm font-medium',
          ]"
          @click="navigate"
        >
          <Component
          ...
          <span
            ...
            class="hover:opacity-250 ltr:ml-auto rtl:mr-auto flex h-5 w-5 cursor-pointer items-center justify-center rounded bg-gray-700 text-white hover:bg-gray-800 hover:text-gray-300 group-hover:bg-gray-500"
