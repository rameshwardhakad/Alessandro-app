    ...
    <div class="flex">
      <div class="w-64 space-y-6">
        <Card class="overflow-hidden">
          <div class="border-b px-4 py-3.5 text-sm font-medium text-gray-800">
            {{ __('App Settings') }}
          </div>
          <nav class="py-1">
        ...
        <Card class="overflow-hidden">
          <div class="border-b px-4 py-3.5 text-sm font-medium text-gray-800">
            {{ __('Front Site Settings') }}
          </div>
          <nav class="py-1">
            <template v-for="nav in useSettingsNavFront" :key="nav.label">
              <router-link
                v-if="!nav.permission || can(nav.permission)"
                v-slot="{ href, navigate, isActive }"
                :to="nav.uri"
...
<script setup lang="ts">
  import { Card, PageContainer } from 'thetheme'
  import { useSettingsNav, useSettingsNavFront } from 'Use/settings-nav'
</script>
