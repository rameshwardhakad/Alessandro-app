    ...
    <Card class="p-6">
      ...
      >
        ...
        <div class="cursor-pointer px-2" @click="handleInputType">
          ...
          <EyeSlashIcon v-show="inputType == 'password'" class="h-4 w-4 text-gray-500" />
        </div>
      </div>

      <div v-if="errors.purchase_code" class="mt-2 text-xs italic leading-normal text-red-600">
        {{ errors.purchase_code[0] }}
      </div>

      <div
        v-if="updates.update_available"
        class="mb-6 mt-6 flex items-center justify-between rounded-md border border-gray-200 p-4"
      >
        <div>
          {{ __('A new Spack update is available.') }}
          <span class="text-indigo-600">{{ updates.new_version }}</span>
          <p class="mt-1 text-gray-500">
            {{ __('while updating it will down your app and take some time to update.') }}
          </p>
        </div>

        <TheButton white :processing="updateProcessing" :disabled="processing" @click="updateNow">
          {{ __('Update Now') }}
        </TheButton>
      </div>
      ...
      <div
        v-if="uptodate"
        class="mb-6 mt-4 flex items-center justify-between rounded-md border border-gray-200 p-4"
      >
...
<script setup lang="ts">
  import { axios, can, useFlashStore } from 'spack'
  import { ref } from 'vue'
