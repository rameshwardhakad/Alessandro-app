...
<script setup lang="ts">
  ...
  const form = useFormStore<any>('setting-front-general')(),
    width = 1600,
    height = 1200,
    src = ref<any | null>(null)

  src.value = form.data.front_site_logo ? `/${form.data.front_site_logo}` : null

  function remove() {
    src.value = null
    form.data.front_site_logo = null
  }
    ...
    reader.onload = (e) => {
      ...
      img.onload = function () {
        ...
        axios.post('logo', formData).then(({ data }) => {
          form.data.front_site_logo = data
        })
      }
    }
  }
